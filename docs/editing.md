...
- **Colors.** The editor's colors, including the selection, matching occurrences of the selected text, and search matches, come from the package's CSS custom properties mapped to the app palette in `css/editor.css`. Highlights are translucent layers over the editor background, so dark mode sets stronger values. The active line must stay translucent too, because it is painted over the selection layer. Tests capture the window and fail if a mouse selection does not visibly change the pixels, if the selection's contrast drops below 1.8:1, or if the active line is opaque.
